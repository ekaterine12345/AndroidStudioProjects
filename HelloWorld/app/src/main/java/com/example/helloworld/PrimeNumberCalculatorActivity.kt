...
class PrimeNumberCalculatorActivity : AppCompatActivity() {
    ...
    buttonCalculate.setOnClickListener{
        ...
        if (m<n) Toast.makeText(this,"first number must be less than second",Toast.LENGTH_SHORT).show()
        else
        if (m<=0 || n<=0) Toast.makeText(this,"numbers must be positive integers",Toast.LENGTH_SHORT).show()
        else
        {
        if (n==1) n++
